...
var gsel = {{}};  // 選択中の学年(複数可)。空なら全学年
function selEmpty(){{ return Object.keys(gsel).length === 0; }}
...
function apply(){{
  ...
  tiles.forEach(function(t){{
    var ok = (!v || h2h(t.dataset.k).indexOf(v) !== -1) && (selEmpty() || gsel[t.dataset.g]);
...
chips.forEach(function(c){{
  c.addEventListener('click', function(){{
    var g = c.dataset.g;
    if (!g) {{ gsel = {{}}; }}                 // 「すべて」で選択解除
    else if (gsel[g]) {{ delete gsel[g]; }}    // タップで個別にON/OFF(複数選択可)
    else {{ gsel[g] = true; }}
    chips.forEach(function(x){{
      var xg = x.dataset.g;
      x.className = 'chip' + ((xg ? gsel[xg] : selEmpty()) ? ' on' : '');
    }});
    apply();
  }});
}});
